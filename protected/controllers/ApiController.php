...
class ApiController extends CController
{
	...
	protected function getRef($fhir_resource_type, $fhir_id, $checkExists = true)
	{
		if (!($id = \Service\FhirId::parse($fhir_id))) {
			$this->sendResource(
				Service\OperationOutcome::singleIssue(
					FhirValueSet::ISSUESEVERITY_FATAL,
					FhirValueSet::ISSUETYPE_PROCESSING_NOT_SUPPORTED,
		...
		if (!($resource_type = Yii::app()->fhirMap->getInternalResourceType($fhir_resource_type, $id->getPrefix()))) {
			$this->sendResource(
				Service\OperationOutcome::singleIssue(
					FhirValueSet::ISSUESEVERITY_FATAL,
					FhirValueSet::ISSUETYPE_PROCESSING_NOT_SUPPORTED,
					"Unknown ID prefix for resource type '{$resource_type}': '{$id->getPrefix()}'"
				),
				404
			);
		}

		$ref = new \Service\ResourceReference($resource_type, $id->getInternalId());
